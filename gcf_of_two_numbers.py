...
def main():
    ...
    try:
        ...
        user_num_2 = int(user_num_2_string)
        # sets a range.
        if user_num_int and user_num_2 >= 0:
               # calculates the gcf of the two numbers user has entered and then
               # displays it
              for counter in range(user_num_int, 0, -1):
                if user_num_int % counter == 0 and user_num_2 % counter == 0:
                  print("The GCF of {} and {} is {}."
                            .format(user_num_int, user_num_2, counter))
              print("")
              ask_again()
        else:
